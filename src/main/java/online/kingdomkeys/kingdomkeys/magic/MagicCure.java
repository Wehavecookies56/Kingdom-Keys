...
public class MagicCure extends Magic {
	...
	protected void magicUse(Player player, Player caster, int level, float fullMPBlastMult, LivingEntity lockOnEntity) {
		...
		case 0:
			...
			break;
		case 1:
			player.level().playSound(null, player.position().x(),player.position().y(),player.position().z(), ModSounds.cura.get(), SoundSource.PLAYERS, 1F, 1F);
			player.heal(amount);
			...
			break;
		case 2:
			player.level().playSound(null, player.position().x(),player.position().y(),player.position().z(), ModSounds.curaga.get(), SoundSource.PLAYERS, 1F, 1F);
			player.heal(amount);
			...
			break;
		case 3:
			player.level().playSound(null, player.position().x(),player.position().y(),player.position().z(), ModSounds.curaga.get(), SoundSource.PLAYERS, 1F, 0.9F);
			player.heal(amount);
		            ...
		            for (int i = 0; i < list.size(); i++) {
		            	if(player.level().getPlayerByUUID(list.get(i).getUUID()) != null && player.distanceTo(player.level().getPlayerByUUID(list.get(i).getUUID())) < ModConfigs.partyRangeLimit) {
			                ...
			                if (e != null && Utils.isEntityInParty(party, e) && e != player) {
			            		...
			            		player.level().playSound(null, e.position().x(),e.position().y(),e.position().z(), ModSounds.curaga.get(), SoundSource.PLAYERS, 1F, 0.9F);
			                }
		            	}
		            }
		        }
