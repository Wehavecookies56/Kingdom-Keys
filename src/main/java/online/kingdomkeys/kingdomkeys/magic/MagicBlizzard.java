...
public class MagicBlizzard extends Magic {
	...
	protected void magicUse(PlayerEntity player, PlayerEntity caster, int level) {
		switch(level) {
		case 0:
			ThrowableEntity blizzard = new BlizzardEntity(player.world, player, getDamageMult(level));
		...
		case 1://-ra and -ga are dmg boosted here
			for(int i = -1; i < 2; i++) {
				ThrowableEntity blizzara = new BlizzardEntity(player.world, player, getDamageMult(level)* 1.3F);
		...
		case 2:
			for(int i = -1; i < 2; i++) {
				ThrowableEntity blizzara = new BlizzardEntity(player.world, player, getDamageMult(level)*1.6F);
				player.world.addEntity(blizzara);
				blizzara.setDirectionAndMovement(player, player.rotationPitch, player.rotationYaw + i*6, 0, 2F, 0);
				player.world.playSound(null, player.getPosition(), SoundEvents.BLOCK_GLASS_BREAK, SoundCategory.PLAYERS, 1F, 1F);
			}
			...
			break;
		case 3:
			BlizzazaEntity blizzaza = new BlizzazaEntity(player.world, player, getDamageMult(level));
