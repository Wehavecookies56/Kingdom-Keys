...
public class GuiMenu_Synthesis_Synthesise_Items extends GuiMenu_Background {
	
	GuiMenuButton[] items = new GuiMenuButton[Lists.keybladeRecipes.size()+Lists.itemRecipes.size()];
	Button prev,next, create;
	int selectedItem = -1;
	int page = 0;
	List<String> itemRecipes = new ArrayList<String>();

		
	public GuiMenu_Synthesis_Synthesise_Items() {
		super(Strings.Gui_Synthesis_Synthesise_Items, new Color(0, 255, 0));
		drawPlayerInfo = true;
	}
	...
	protected void action(String string) {
		if(string.startsWith("s:")) {
			...
			selectedItem = getIndexFromName(data[1]);
		} else {
			switch(string) {
			case "prev":
				page--;
			...
			case "create":
				...
				PacketHandler.sendToServer(new CSSynthesiseKeyblade(itemRecipes.get(selectedItem)));
	...
	private int getIndexFromName(String name) {
		...
		for(int i = 0;i < itemRecipes.size(); i++) {
			if(itemRecipes.get(i).equals(name)) {
				return i;
			}
		}
		return -1;
	...
	private void updateButtons() {
		...
		if(selectedItem > -1) {
			IPlayerCapabilities playerData = ModCapabilities.getPlayer(minecraft.player);
			List<String> recipeList = itemRecipes;
			String itemFromList = recipeList.get(selectedItem);
			String name = itemFromList.substring("item.kingdomkeys.".length());
	...
	public void init() {
		...
		for(int i = 0;i<itemRecipes.size();i++) {
			String name = itemRecipes.get(i);
			Recipe recipe = RecipeRegistry.getInstance().getValue(new ResourceLocation(name.substring(5).replace(".", ":")));

			if(recipe != null) {
				addButton(items[i] = new GuiMenuButton((int) buttonPosX, button_statsY + (i * 18), (int) buttonWidth, Utils.translateToLocal(itemRecipes.get(i)), ButtonType.BUTTON, (e) -> { action("s:"+name); }));
				items[i].active = RecipeRegistry.getInstance().getValue(new ResourceLocation(name.substring(5).replace(".", ":"))) != null;
			} 
		}
	...
	public void render(int mouseX, int mouseY, float partialTicks) {
		...
		IPlayerCapabilities playerData = ModCapabilities.getPlayer(minecraft.player);
		prev.visible = page > 0;
		next.visible = page < itemRecipes.size()/8;
		...
		for(int i=page*8;i< page*8+8;i++) {
			if(i < Lists.keybladeRecipes.size() && items[i] != null) {
				items[i].visible = true;
				items[i].y = ((i/8)-(i/8)+3+(i%8))*18;
				items[i].x = (int) (width * 0.04F);
			}
		}
		...
		if(selectedItem > -1) {
			RenderSystem.pushMatrix();
			{
				String kb = itemRecipes.get(selectedItem);
				...
				{
					RenderSystem.scaled(1,1,1);
					String text = Utils.translateToLocal(itemRecipes.get(selectedItem));
