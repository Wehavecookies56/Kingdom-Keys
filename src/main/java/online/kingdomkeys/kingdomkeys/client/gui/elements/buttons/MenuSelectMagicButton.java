		...
		super(x, y, widthIn, 20, "", b -> {
			if (b.visible && b.active) {
				if (slot != -1) {
					...
					if (!NeoForge.EVENT_BUS.post(new EquipmentEvent.Armour(player, playerData.getEquippedMagic(parent.slot), player.getInventory().getItem(slot), slot, parent.slot)).isCanceled()) {
