...
public class GuiMenu_Synthesis_Synthesise_Keyblades extends GuiMenu_Background {
	
	GuiMenuButton[] keyblades = new GuiMenuButton[Lists.keybladeRecipes.size()+Lists.itemRecipes.size()];
	...
	public void init() {
		...
		for(int i = 0;i<keybladeRecipes.size();i++) {
			...
			if(recipe != null) {
				addButton(keyblades[i] = new GuiMenuButton((int) buttonPosX, button_statsY + (i * 18), (int) buttonWidth, Utils.translateToLocal(keybladeRecipes.get(i)), ButtonType.BUTTON, (e) -> { action("s:"+name); }));
