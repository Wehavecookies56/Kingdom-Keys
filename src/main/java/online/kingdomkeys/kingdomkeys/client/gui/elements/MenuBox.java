...
public class MenuBox extends AbstractWidget{
    ...
    public MenuBox(int posX, int posY, int width, int height, float alpha, Color colour) {
	...
	public void renderWidget(GuiGraphics gui, int p_268034_, int p_268009_, float p_268085_) {
        ...
        {
            RenderSystem.setShaderColor(colour.getRed() / 255F, colour.getGreen() / 255F, colour.getBlue() / 255F, alpha);
            ...
            matrixStack.pushPose();
            {
                ...
                gui.blit(Constants.MENU_TEXTURE, 0, 0, mCenterU, mCenterV, 1, 1);
            }
            matrixStack.popPose();
