...
import uk.co.wehavecookies56.kk.common.network.packet.server.EquipPotion;
import uk.co.wehavecookies56.kk.common.util.Utils;

public class GuiPotionListItem extends GuiButton {
	...
	public GuiPotionListItem(ItemStack item, int slot, int x, int y, GuiItems parent, int colour) {
		super(0, x, y, "");
		this.stack = item;
	...
	public void drawButton(Minecraft mc, int mouseX, int mouseY, float partialTicks) {
		...
		ItemKKPotion item = (ItemKKPotion) stack.getItem();
		if (visible) {
			...
			if (item == null) {
				...
				itemName = new ItemStack(item).getDisplayName();
			}
		...
		if (visible && enabled) {
			if (mouseX >= x && mouseX <= x + width) {
				if (mouseY >= y && mouseY <= y + height) {
					...
					PacketDispatcher.sendToServer(new EquipPotion(stack, slot, parent.itemSlot));
