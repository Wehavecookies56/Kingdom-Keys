...
public class GUIMenuO extends Screen{
	...
	public void render(int mouseX, int mouseY, float partialTicks) {
		...
		text = "Experience: "+props.getExperience();
		drawCenteredString(Minecraft.getInstance().fontRenderer, text, this.width / 2 - text.length()/2, 110, 0xFFFFFF);
		text = "Strength: "+props.getStrength();
		drawCenteredString(Minecraft.getInstance().fontRenderer, text, this.width / 2 - text.length()/2, 120, 0xFFFFFF);
		text = "Magic: "+props.getMagic();
		drawCenteredString(Minecraft.getInstance().fontRenderer, text, this.width / 2 - text.length()/2, 130, 0xFFFFFF);
		text = "Defense: "+props.getDefense();
		...
		text = "DP: "+props.getDP()+"/"+props.getMaxDP()+" "+ props.getFP();
		drawCenteredString(Minecraft.getInstance().fontRenderer, text, this.width / 2 - text.length()/2, 170, 0xFFFFFF);
        }
