...
public class GuiMenu_Synthesis_Synthesise_Items extends MenuBackground {
	
	MenuButton[] items = new MenuButton[Lists.keybladeRecipes.size()+Lists.itemRecipes.size()];
	...
	public void init() {
		...
		for(int i = 0;i<itemRecipes.size();i++) {
			...
			if(recipe != null) {
				addButton(items[i] = new MenuButton((int) buttonPosX, button_statsY + (i * 18), (int) buttonWidth, Utils.translateToLocal(itemRecipes.get(i)), ButtonType.BUTTON, (e) -> { action("s:"+name); }));
