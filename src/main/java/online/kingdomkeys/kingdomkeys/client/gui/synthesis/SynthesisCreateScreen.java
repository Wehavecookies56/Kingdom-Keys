...
public class SynthesisCreateScreen extends MenuFilterable {
	...
	public void init() {
		...
		// addWidget(scrollBar = new MenuScrollBar());
		super.init();
	...
	public void initItems() {
		...
		addWidget(create = new Button((int) (boxM.x+3), (int) (height * 0.67), boxM.getWidth()-5, 20, new TranslatableComponent(Utils.translateToLocal(Strings.Gui_Synthesis_Synthesise_Create)), (e) -> {
			action("create");
		}));
		
		addWidget(back = new MenuButton((int)this.buttonPosX, this.buttonPosY, (int)buttonWidth/2, new TranslatableComponent(Strings.Gui_Menu_Back).getString(), MenuButton.ButtonType.BUTTON, b -> minecraft.setScreen(new SynthesisScreen())));
	...
	protected void renderSelectedData(PoseStack matrixStack, int mouseX, int mouseY, float partialTicks) {
		...
		RenderSystem.pushMatrix();
		{
			...
			if(RecipeRegistry.getInstance().containsKey(selected.getItem().getRegistryName())) {
				...
				drawString(matrixStack, minecraft.font, line, boxM.getWidth() - minecraft.font.width(line) - 10, -8, recipe.getCost() > playerData.getMunny() ? Color.RED.getRGB() : Color.GREEN.getRGB());
			}
			...
			itemRenderer.renderGuiItem(selected, 2, 3);
		}
		...
		if (selected.getItem() != null && selected.getItem() instanceof KeybladeItem) {
			...
			matrixStack.pushPose();
			{
				...
				if(nextAbility != null) {
					...
					drawString(matrixStack, minecraft.font, abilityHeader, -20 + (boxM.getWidth()/2) - (minecraft.font.width(abilityHeader)/2), 0, 0xFFFF44);
					...
					if(a != null) {
						...
						drawString(matrixStack, minecraft.font, abilityName, -20 + (boxM.getWidth()/2) - (minecraft.font.width(abilityName)/2), 10, 0x44FF44);
						offset = -20;
					}
				}
				drawString(matrixStack, minecraft.font, Utils.translateToLocal(Strings.Gui_Menu_Status_Strength)+": "+kb.getStrength(0), 0, offset, 0xFF0000);
				drawString(matrixStack, minecraft.font, Utils.translateToLocal(Strings.Gui_Menu_Status_Magic)+": "+kb.getMagic(0), 0, offset+10, 0x4444FF);
