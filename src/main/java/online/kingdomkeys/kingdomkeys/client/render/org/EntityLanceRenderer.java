...
@OnlyIn(Dist.CLIENT)
public class EntityLanceRenderer extends EntityRenderer<LanceEntity> {

	public static final Factory FACTORY = new EntityLanceRenderer.Factory();
	Random rand = new Random();
	float rotation = 0;
	
	public EntityLanceRenderer(EntityRendererManager renderManager) {
		super(renderManager);
	...
	public static class Factory implements IRenderFactory<LanceEntity> {
		...
		public EntityRenderer<? super LanceEntity> createRenderFor(EntityRendererManager manager) {
			return new EntityLanceRenderer(manager);
		}
	}
}
