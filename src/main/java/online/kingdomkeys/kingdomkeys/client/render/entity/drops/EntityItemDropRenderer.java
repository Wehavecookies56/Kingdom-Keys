...
public class EntityItemDropRenderer extends EntityRenderer<EntityItemDrop> {
	...
	protected int getBlockLight(EntityItemDrop entityIn, float partialTicks) {
		return MathHelper.clamp(super.getBlockLight(entityIn, partialTicks) + 7, 0, 15);
	}

	public void render(EntityItemDrop entityIn, float entityYaw, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn) {
		matrixStackIn.push();
		float f = 0;
		float f1 = 1F;
		float f2 = 0;
