...
public class ShotlockMultivortex extends Shotlock {
	private static final int SHOT_COLOR = 0xFFF0C0;

	public ShotlockMultivortex(ResourceLocation registryName, int order) {
		super(registryName, order);
	}

	@Override
	public void onUse(Player player, List<Entity> targetList) {
		player.level().playSound(null, player.position().x(), player.position().y(), player.position().z(), ModSounds.portal.get(), SoundSource.PLAYERS, 1F, 1.5F);
	...
	public void doPartialShotlock(Player player, List<Entity> targetList) {
		...
		if(player.getMainHandItem() != null && player.getMainHandItem().getItem() instanceof KeybladeItem kb){
			core.setVisualItem(player.getMainHandItem());
		} else {
			core.setVisualItem(new ItemStack(Items.IRON_SWORD));
		}
