...
public class ShotlockLightningRay extends Shotlock {

	private static final Color TRAIL_COLOR = new Color(255, 226, 74);

	public ShotlockLightningRay(ResourceLocation registryName, int order) {
		super(registryName, order);
	}

	@Override
	public void onUse(Player player, List<Entity> targetList) {
		player.level().playSound(null, player.position().x(), player.position().y(), player.position().z(), ModSounds.portal.get(), SoundSource.PLAYERS, 1F, 1.3F);
