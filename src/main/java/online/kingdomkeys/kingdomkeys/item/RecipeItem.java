		...
		for (ResourceLocation recipe : recipes) {
			if (RecipeRegistry.getInstance().containsKey(recipe)) {
				...
				if (!RecipeRegistry.getInstance().containsKey(recipe)) { // If recipe is not valid
					player.sendSystemMessage(Component.literal("ERROR: Recipe for " + recipe + " was not learnt because it is not a valid recipe, report this to a dev").withStyle(ChatFormatting.RED));
				} else if (playerData.hasKnownRecipe(recipe)) { // If recipe already known
					player.sendSystemMessage(Component.translatable("message.recipe.already_learnt", outputStack.getHoverName().copy().withStyle(ChatFormatting.YELLOW)));
				} else { // If recipe is not known, learn it
					playerData.addKnownRecipe(recipe);
					consume = true;
					player.sendSystemMessage(Component.translatable("message.recipe.learnt", outputStack.getHoverName().copy().withStyle(ChatFormatting.GREEN)));
