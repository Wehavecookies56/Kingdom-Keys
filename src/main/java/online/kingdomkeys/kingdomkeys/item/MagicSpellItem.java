...
public class MagicSpellItem extends Item implements IItemCategory {
		...
		if (!world.isRemote) {
			...
			if (playerData != null && playerData.getMagicList() != null) {
				if (!playerData.getMagicList().contains(magic)) {
					...
					player.sendMessage(new TranslationTextComponent("Unlocked " + Utils.translateToLocal(magic.substring(magic.indexOf(":") + 1))));
				} else {
					player.sendMessage(new TranslationTextComponent(Utils.translateToLocal(magic.substring(magic.indexOf(":") + 1)) + " Already unlocked"));
				}
	...
	public void addInformation(ItemStack stack, World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn) {
		tooltip.add(new TranslationTextComponent("Unlock " + Utils.translateToLocal(magic.substring(magic.indexOf(":") + 1))));
