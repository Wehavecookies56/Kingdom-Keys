...
import online.kingdomkeys.kingdomkeys.entity.organization.CardRingCoreEntity;
import online.kingdomkeys.kingdomkeys.item.organization.IOrgWeapon;
import online.kingdomkeys.kingdomkeys.lib.DamageCalculation;
import online.kingdomkeys.kingdomkeys.util.Utils.OrgMember;

public class LimitCardRing extends Limit {

	public LimitCardRing(ResourceLocation registryName, int order, OrgMember owner) {
	...
	public void onUse(Player player, LivingEntity target) {
		...
		player.level().playSound(null, player.position().x(), player.position().y(), player.position().z(), ModSounds.portal.get(), SoundSource.PLAYERS, 1F, 0.9F);
		...
		CardRingCoreEntity ring = new CardRingCoreEntity(player.level(), player, damage);
