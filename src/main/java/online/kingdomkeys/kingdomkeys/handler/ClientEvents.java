...
public class ClientEvents {
			...
			if(event.getEntity() instanceof Player) {
				...
				if(playerData != null) {
					...
					if(playerData.getAerialDodgeTicks() > 0) {
						event.getMatrixStack().mulPose(Vector3f.YP.rotationDegrees(player.tickCount*80));
					}
	...
	public void PlayerTick(PlayerTickEvent event) {
		if (event.phase == Phase.END) {
			...
			if (event.player == mc.player && cooldownTicks <= 0) { // Only run this for the local client player
				focusing = mc.options.keyPickItem.isDown() && event.player.getMainHandItem() != null && Utils.getPlayerShotlock(mc.player) != null && (event.player.getMainHandItem().getItem() instanceof KeybladeItem || event.player.getMainHandItem().getItem() instanceof OrgWeaponItem);
		...
		if (mc.player != null && ModCapabilities.getPlayer(mc.player) != null) {
			...
			if (playerData.getShotlockEnemies() != null && playerData.getShotlockEnemies().contains(event.getEntity().getId())) {
				...
				if(true) {
					...
					EntityRenderDispatcher renderManager = event.getRenderer().getDispatcher();
					...
					matrixStackIn.pushPose();
					{
						...
						matrixStackIn.scale(-scale, -scale, scale);
						event.getRenderer().getDispatcher().textureManager.bindForSetup(new ResourceLocation(KingdomKeys.MODID, "textures/gui/focus2.png"));
						blit(matrixStackIn,-128,-128,0,0,256,256);
					}
	...
	public void PlayerClick(ClickInputEvent event) {
		if(event.isPickBlock()) {
			Minecraft mc = Minecraft.getInstance();
			if(mc.player.getMainHandItem() != null && Utils.getPlayerShotlock(mc.player) != null && (mc.player.getMainHandItem().getItem() instanceof KeybladeItem || mc.player.getMainHandItem().getItem() instanceof OrgWeaponItem)){
				event.setCanceled(true);
			}
		}	
	}
