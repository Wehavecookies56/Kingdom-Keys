...
public class MobSpawnings {
	...
	public static void registerSpawns(BiomeLoadingEvent event) {
		...
		for (EntityType<?> entityType : ModEntities.pureblood) {
			event.getSpawns().getSpawner(entityType.getCategory()).add(new MobSpawnSettings.SpawnerData(entityType, Integer.parseInt(pureblood[1]), Integer.parseInt(pureblood[2]), Integer.parseInt(pureblood[3])));
		}
		for (EntityType<?> entityType : ModEntities.emblem) {
			event.getSpawns().getSpawner(entityType.getCategory()).add(new MobSpawnSettings.SpawnerData(entityType, Integer.parseInt(emblem[1]), Integer.parseInt(emblem[2]), Integer.parseInt(emblem[3])));
		}
		for (EntityType<?> entityType : ModEntities.nobody) {
			event.getSpawns().getSpawner(entityType.getCategory()).add(new MobSpawnSettings.SpawnerData(entityType, Integer.parseInt(nobody[1]), Integer.parseInt(nobody[2]), Integer.parseInt(nobody[3])));
		}
	}
