...
public class DriveFormFinal extends DriveForm {
	...
	public static void getFinalFormXP(LivingDeathEvent event) {
		if (!event.getEntity().level.isClientSide && (event.getEntity() instanceof EnderMan) || event.getEntity() instanceof IKHMob && ((IKHMob)event.getEntity()).getEnemyType() == MobType.NOBODY) {
