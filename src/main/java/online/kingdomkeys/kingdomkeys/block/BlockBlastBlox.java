...
public class BlockBlastBlox extends BlockBase {
    ...
    private void explode(World world, BlockPos pos, @Nullable LivingEntity igniter) {
        if (!world.isRemote) {
            EntityBlastBloxPrimed entity = new EntityBlastBloxPrimed(ModEntities.TYPE_BLAST_BLOX, world, (double)((float)pos.getX() + 0.5F), (double)pos.getY(), (double)((float)pos.getZ() + 0.5F), igniter);
